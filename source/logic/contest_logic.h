...
/// @brief Creates ContestPlayer structs for each player. Also checks the status of the wake up test.
/// Players that can't pass the wake up test are added to the contest, but don't compete.
...
/// @brief Wakes every player process up to make sure they can connect to a socket and send a hello message.
...
/// @brief Manages the contest at the round level. This is the main loop of the contest (It's a do-while loop baby).
...
/// @brief Iterates over all players in the contest and creates a working list of living players.
...
/// @brief Randomly selects from all round players and chooses who to play against.
...
/// @brief Collects all stats, errors, and sets lives based on losses/ties. Collects from contest match player into contest player.
...
/// @brief Manages a single match in a round. Also manages displaying a match if applicable.
/// @param c_match ContestMatch struct to store MatchLog info into.
/// @param connect Connection struct to use during match.
/// @param contest_options Options to use during contest. Applied to match options.
/// @param socket_name Name of socket to connect over.
