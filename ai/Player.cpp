 ...
 * @brief The Implementation of the Base Class for the Player AIs.
 * @date 2026-06-02
 */
...
int Player::play_match(char *socket_path, const char *ai_name, const char *author_names) {
    if (!connect_to_socket(socket_path)) return -1;

    // hello code
    message_hello_create(ai_name, author_names);
    if (!message_send()) return -1;

    // setup match code
    {
        if (!message_receive()) return -1;
    ...
    while (type != MESSAGE_MATCH_OVER) {
        ...
        case MESSAGE_PLACE_SHIPS:
            ...
            for (int i = 0; i < (int)j[LENGTH_KEY].size(); i++) {
                ship1 = choose_ship_place((int)j[LENGTH_KEY].at(i));
...
bool Player::connect_to_socket(char *socket_path) {
...
void Player::message_hello_create(const char *ai_name, const char *author_names) {
...
void Player::message_ships_placed_create(vector<Ship> ships) {
    json j = {
...
void Player::message_shot_taken_create(Shot shot) {
    json j = {
